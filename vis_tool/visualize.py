...
def main():
    #input parameter
    path_to_images = "/root/python/OpenCV/data/lmo_test_all/test/000002/rgb"
    image_extension = ".png"
    phi = 0
    ...
    # 既に推論済みのものを可視化
    inferred_file_path="../zebrapose/path/to/save/the/evaluation/report/pose_result_bop/lmo_ape.csv"
    inferred=[]
    ...
    for row in tqdm(inferred):
        ...
        scores=np.array([float(row[2])])
        print(scores)
        labels=np.array([0])

        # リストをNumPy配列に変換し、形状を変更
        rot_elements = [float(x) for x in row[4].split()]
        rotations=np.array(rot_elements).reshape((1,3,3))
        print(rotations)
        print(rotations[0].shape)

        trans_elements=[float(x) for x in row[5].split()]
        translations=np.array(trans_elements).reshape((1,3,1))
        print(translations)
